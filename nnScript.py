...
def preprocess():
    ...
    return train_data, train_label, validation_data, validation_label, test_data, test_label, chosenInd
...
#     obj_val = 0
    ...
    no_of_classes = 10
    rI = np.arange(len(train_label),dtype="int")
    t = np.zeros((len(train_label),no_of_classes))
    ...
    val_t1 = t*np.log(Y)
    val_t2 = t_1*np.log(Y_1)
    val_t1 = np.sum(-1*(val_t1+val_t2))
    val_t1 = val_t1/trainN
    
    w1_w1 = np.square(w1)
    w2_w2 = np.square(w2)
    w1_w2 = np.sum(w1_w1) + np.sum(w2_w2)
    n2 = 2*trainN
    val_t2 = (lambdaval/n2) * w1_w2
    obj_val = val_t1 + val_t2
...
t_start = time.time()
train_data, train_label, validation_data, validation_label, test_data, test_label, chosenInd = preprocess()
...
pickle.dump((lambdaval,n_hidden,w1,w2,chosenInd),open('params.pickle','wb'))
